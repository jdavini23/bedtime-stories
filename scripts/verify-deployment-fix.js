...
const log = {
  success: (msg) => console.log(chalk?.green ? chalk.green('✓ ' + msg) : `${colors.green}✓ ${msg}${colors.reset}`),
  error: (msg) => console.log(chalk?.red ? chalk.red('✗ ' + msg) : `${colors.red}✗ ${msg}${colors.reset}`),
  warning: (msg) => console.log(chalk?.yellow ? chalk.yellow('! ' + msg) : `${colors.yellow}! ${msg}${colors.reset}`),
  info: (msg) => console.log(chalk?.cyan ? chalk.cyan('ℹ ' + msg) : `${colors.cyan}ℹ ${msg}${colors.reset}`),
  header: (msg) => console.log(chalk?.bold ? chalk.bold(msg) : `${colors.bright}${msg}${colors.reset}`),
};
...
try {
  ...
  if (packageJson.dependencies['@opentelemetry/api'] === '1.8.0') {
    ...
    log.error(`@opentelemetry/api version is ${packageJson.dependencies['@opentelemetry/api']} but should be 1.8.0`);
    hasErrors = true;
  }
...
try {
  if (fs.existsSync(vercelJsonPath)) {
    ...
    if (vercelJson.installCommand === 'npm ci --legacy-peer-deps') {
      ...
      log.error(`vercel.json installCommand is "${vercelJson.installCommand}" but should be "npm ci --legacy-peer-deps"`);
      hasErrors = true;
    }
  } else {
