...
function StepCard({ number, title, description, image, color }: StepCardProps) {
  ...
  return (
    <div className="relative">
      <div
        className={`absolute top-0 left-1/2 transform -translate-x-1/2 -translate-y-1/2 ${colorMap[color as keyof typeof colorMap].bg} text-white h-12 w-12 rounded-full flex items-center justify-center text-xl font-bold shadow-lg z-10`}
      >
        {number}
      </div>
      <div
        className={`h-1 ${colorMap[color as keyof typeof colorMap].gradient} absolute top-0 left-1/2 w-full`}
      ></div>
